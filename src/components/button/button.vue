<template>
  ...
  >
    ...
    <i :class="['b-iconfont','b-icon-'+icon]" v-if="icon && !loading" :style="iconStyles"></i>
...
import { ComponentSize } from '../../utils/type'

type IButtonType = PropType<'primary' | 'success' | 'warning' | 'danger' | 'info' | 'text' | 'dashed' | 'default'>
...
export default defineComponent({
  ...
  props: {
    type: {
      ...
      default: 'default',
      validator: (val: string) => ['default', 'primary', 'success', 'warning', 'info', 'danger', 'dashed', 'text'].includes(val),
    },
    size: {
      type: String as ComponentSize,
      validator: (val: string) => ['default', 'large', 'small', 'mini'].includes(val),
      default: 'default',
    },
    icon: String,
    iconStyle: Object,
